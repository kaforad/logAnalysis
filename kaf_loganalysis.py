...
#Write report output to text file
	...
	emptyStri = " ";
	#loop through list value and concantenate list value to make a line of report
...
def authorPopularity():
	...
	authors join (select author,title from articles group by author,title) as Aarticles
	on authors.id= Aarticles.author join 
	(Select title, logs.popularity from articles join (select reverse(split_part(reverse(path), '/', 1)) as path, count(*) as popularity from log  group by path having path like '%article%'order by popularity  desc ) as logs 
...
def errorPopularity():
	conn = psycopg2.connect(dbname="news");
	cursor= conn.cursor();
	myquery = """select TO_CHAR(time, 'FMMonth FMDDth, YYYY') as time,(ROUND(count(*)*100.0/(select count(*) from log where status<>'200 OK' group by status),2)) as percent from log where status<>'200 OK' group by TO_CHAR(time, 'FMMonth FMDDth, YYYY'),method,status having (count(*)*100.0/(select count(*) from log where status<>'200 OK' group by status))>1 order by time desc ;  """ 
	cursor.execute(myquery);
	results = cursor.fetchall();
	report ={};
	i =0;
